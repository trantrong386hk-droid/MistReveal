...
class TextGenerationService {
    ...
    - 禁用词：星辰、宿命、银河、轮回、天命、前世、来生、缘定三生、冥冥之中、冥冥中、注定；韩系、网红、偶像、奶油、娃娃脸、初恋脸、氧气感

    【角色与性格】
    用日主+十神+五行旺衰写出"性格明暗面"与"情感需求"，语气冷静克制但有洞察。不要玄学堆砌。

    【伴侣画像原则】
    - 伴侣五行 = 喜用神，体现为“能量感受”，不要用元素字面颜色
    - “木的能量”用质感与光影表达（清爽、通透、呼吸感），不要使用绿植/森林/苔藓意象
    - 视觉重点：骨相结构、体态张力、眼神神态、皮肤质感
    - 服饰遵守「服饰审美区间」，只在给定区间内选择材质与色调
    - 发色必须自然黑/深棕，不染发、不挑染，不受环境色反光影响
    - 五行能量只体现在气质、光影与环境，不体现在发色或瞳色
    - 光线尽量中性暖光，背景优先选择中性室内/城市质感场景，避免大量绿植

    【输出风格】
    赛博玄学感、逻辑锋利、场景化洞察；避免抒情和空洞标签。
    ...
    private func elementEnergyDescription(_ element: String, soulmateGender: String) -> String {
        ...
        case "木":
            if isMale {
                ...
                服饰审美（男性）：粗纺棉麻、做旧帆布、自然肌理的面料、松弛的廓形。色调从原木色、燕麦色、沙石色中选择（避免绿色）。
                光线环境：中性暖光，背景以室内/城市质感场景为主，避免大量绿植。
                """
            } else {
                return """
                用户命局缺生机，伴侣应传递「蓬勃生命力」的视觉感受。
                服饰审美（女性）：轻盈棉麻、自然肌理的编织面料、透气有层次的叠穿。色调从奶油白、杏色、浅沙色中选择（避免绿色）。
                光线环境：中性暖光，背景以室内/城市质感场景为主，避免大量绿植。
                """
            }
        case "火":
        ...
        case "水":
            if isMale {
                ...
                服饰审美（男性）：有垂感的软质面料、暗纹肌理、不张扬的高级质感。色调从墨色、深灰、石墨灰、深炭色中选择。
                光线环境：柔和低饱和光线，室内/城市质感场景为主，避免强色环境。
                """
            } else {
                return """
                用户命局缺柔韧，伴侣应传递「沉静疏解」的视觉感受。
                服饰审美（女性）：有垂感的重磅真丝（避开廉价反光）、层叠薄纱、流动的裙摆。色调从烟灰、暖灰、月白、深黛灰中选择。
                光线环境：柔和低饱和光线，室内/城市质感场景为主，避免强色环境。
                """
            }
        case "土":
